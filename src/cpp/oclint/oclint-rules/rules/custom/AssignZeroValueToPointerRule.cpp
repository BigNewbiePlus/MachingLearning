...
class AssignZeroValueToPointerRule : public AbstractASTVisitorRule<AssignZeroValueToPointerRule>
{
public:
    virtual const string name() const override
    {
        return "assign zero value to pointer";
    }
    ...
    bool VisitBinaryOperator(BinaryOperator *node)
    {
        if(node->getOpcode()==BO_Assign){
            ...
            if(isZeroValue(rhs) && isPointer(lhs, pType)){
                ...
                string message = "It is odd that the '\\0' value is assigned to '"+
                    pType+"' type pointer. Probably meant: *"+exprStr+".";
...
static RuleSet rules(new AssignZeroValueToPointerRule());    