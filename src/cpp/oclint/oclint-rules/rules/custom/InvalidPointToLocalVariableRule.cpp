...
class InvalidPointToLocalVariableRule : public AbstractASTVisitorRule<InvalidPointToLocalVariableRule>
{
public:
    virtual const string name() const override
    {
        return "invalid point to local variable";
    }
    ...
    bool VisitCompoundStmt(CompoundStmt *node)
    {
        ...
        for(clang::CompoundStmt::body_iterator body_it = node->body_begin(); body_it!=node->body_end();body_it++){
            ...
            if(isa<DeclStmt>(body)){
                ...
                if(binaryOperator->getOpcodeStr()=="=" && isPointerType(lhs)){//赋值运算且左侧为指针变量
                    ...
                    if(localVarNames.find(lname)==localVarNames.end() 
                       && localVarNames.find(rname)!=localVarNames.end()){
                        string message = "Pointer to local variable '"+rname+"' is stored outside the scope of this variable. Such a pointer will become invalid.";
...
static RuleSet rules(new InvalidPointToLocalVariableRule());