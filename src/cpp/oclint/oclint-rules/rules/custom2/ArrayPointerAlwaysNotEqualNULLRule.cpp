...
class ArrayPointerAlwaysNotEqualNULLRule : public AbstractASTVisitorRule<ArrayPointerAlwaysNotEqualNULLRule>
{
public:
    virtual const string name() const override
    {
        return "array pointer always not equal n u l l";
    }
    ...
    */
    ...
    bool VisitBinaryOperator(BinaryOperator *bo)
    {
        ...
        if(bok==BO_EQ || bok==BO_NE){
            if(isArrayPoint(bo->getLHS()) && isZeroInteger(bo->getRHS())){
                string message = "Consider inspecting the condition. The '"+expr2str(bo->getLHS())+"' pointer is always not equal to NULL.";
...
static RuleSet rules(new ArrayPointerAlwaysNotEqualNULLRule());
