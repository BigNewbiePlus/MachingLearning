...
class OwnerlessTokenRule : public AbstractASTVisitorRule<OwnerlessTokenRule>
{
public:
    virtual const string name() const override
    {
        return "ownerless token";
    }
    ...
    */
    ...
    bool VisitCompoundStmt(CompoundStmt *cs)
    {
        for(CompoundStmt::body_iterator it=cs->body_begin();it!=cs->body_end();it++){
            if(isa<CXXBoolLiteralExpr>(*it)){
                string message = "Ownerless token '"+stmt2str(*it)+"'.";
...
static RuleSet rules(new OwnerlessTokenRule());
