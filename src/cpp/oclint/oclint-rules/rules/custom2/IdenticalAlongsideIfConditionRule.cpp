...
class IdenticalAlongsideIfConditionRule : public AbstractASTVisitorRule<IdenticalAlongsideIfConditionRule>
{
public:
    virtual const string name() const override
    {
        return "identical alongside if condition";
    }
    ...
    bool VisitCompoundStmt(CompoundStmt *compoundStmt)
    {
        ...
        for(CompoundStmt::body_iterator it=compoundStmt->body_begin();it!=compoundStmt->body_end(); it++){
            if(isa<IfStmt>(*it)){
                ...
                string condStr = expr2str(ifStmt->getCond());
                cnt[condStr]+=1;
                if(cnt[condStr]==2){
                    string message = "The conditional expressions of the 'if' operators situated alongside each other are identical.";
...
static RuleSet rules(new IdenticalAlongsideIfConditionRule());
